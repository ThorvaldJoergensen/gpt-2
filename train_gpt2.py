...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Attention (materializes the large (T, T) attention matri for all the queries and keys)
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # Create optimizer groups. Any parameter that is 2D will be weight decayed, otherwise no weight decay
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        # print("loaded tokens of shape", self.tokens.shape)
...
# Set up DDP (distributed data parallel) if available
...
# Create model
model = GPT(GPTConfig(vocab_size=50304))  # Nearest power of 2 to 50257
...
def get_lr(it):
    ...
    coeff = 0.5 * (
        1.0 + math.cos(math.pi * decay_ration)
...
for step in range(max_steps):
    ...
    if step % 250 == 0 or last_step:
        ...
        if master_process:
            ...
            if step > 0 and (step % 5000 == 0 or last_step):
                ...
                # You might also want to add optimizer.state_dict() here
                # and rng seeds etc., if you wanted to more exactly resume training
    ...
    if ((step > 0 and step % 250 == 0) or last_step) and (not use_compile):
        ...
        while xgen.size(1) < max_length:
            ...
            with torch.no_grad():
                with torch.autocast(device_type=device_type, dtype=torch.bfloat16):
                    logist, loss = model(xgen)  # (B, T, vocab_size)
                # take the logits at the last position
                logits = logits[:, -1, :]  # (B, vocab_size)
                # get the probabilities
                probs = F.softmax(logits, dim=-1)
                # do top-k sampling of 50 (huggingface default)
                # topk_probs here becomes (B, 50) and topk_indices is (B, 50)
                ...
                # gatcher the corresponding token index
    ...
    for micro_step in range(grad_accum_steps):
        ...
        # Addition of gradients corresponds to a SUM in the objects, but
    ...
    if master_process:
        print(
            f"step {step} | loss: {loss_accum.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tokens/sec: {tokens_per_sec:.2f}"
        )
        with open(log_file, "a") as f:
...
while x.size(1) < max_length:
    # forward to model to get the logits
    with torch.no_grad():
        logits, _ = model(x)  # logits is (B, T, vocab_size)
        # take the logits at the last position
        logits = logits[:, -1, :]  # (B, vocab_size)
        # get the probabilities
        probs = F.softmax(logits, dim=-1)
